...
<tr>
    <td class="image">
        ...
        <div class="image_block">
            ...
            <?php 
				...
				if (file_exists($src) || @getimagesize($src) !== false) {								
					$thumb_img = '<img src="'.$src.'" alt="'.htmlspecialchars($product->name).'" />'; 
					$full_img =  htmlspecialchars($product->image);
				}else { 
					$thumb_img = yt_placehold($placehold_size['product_listing']);
					$full_img  = 'http://placehold.it/'.$placehold_size['product_detail'].'/969696';
		...
		<a title="<?php print htmlspecialchars($product->name);?>" href="<?php echo $full_img;?>" class="zoom" data-rel="prettyPhoto"><i class="fa fa-search"></i></a>
        <?php }?>
